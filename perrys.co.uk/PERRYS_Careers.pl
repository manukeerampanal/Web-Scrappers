...
my $search_url = "http://www.perrys.co.uk/careers";
my $dup_count = 0;
...
my @allLinks = $mech->find_all_links(class => 'btnSlab', url_regex => qr/view\/\d+$/);
foreach my $link (@allLinks){
	...
	my $job_role = $details->look_down(_tag => 'div', id => 'jobRole')->look_down(_tag => 'dl');
	my @dd = $job_role->look_down(_tag => 'dd');
	my $title = $dd[1]->as_trimmed_text;
	my $ref = $dd[0]->as_trimmed_text;
	my $loc = $dd[3]->as_trimmed_text;
	my $description = $dd[2]->as_trimmed_text;
